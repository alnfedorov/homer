...
const char *HomerConfig::homeDirectory = "/home/aleksander/projects/homer/./";
...
PeakLibrary *PeakFinder::findGroSeqTranscripts() {
    ...
    PeakLibrary *regions = tags->findGroSeqTranscripts(style, input, uniqMapDirectory, strand,
                                                       tssSize, minBodySize, maxBodySize, tagThresh,
                                                       foldTranscriptStart,
...
void PeakLibrary::genomeOntology(char **peakFiles, int numPeakFiles, char strand, int maxDistance,
    ...
    if (control != NULL) {
        ...
        fprintf(fp,
                "\tLog Ratio Enrichment(vs. Control)\tLog P-value(+ underrepresented, vs. Control)\tP-value(vs. Control)");
    }
    fprintf(fp, "\n");
    ...
    for (int i = 0; i < numPeakFiles; i++) {
		...
		*/
        ...
        if (maxDistance > 0) {
            ...
            long long int expOverlap = (long long int) (((double) totalCoverage) * ((double) curCoverage) /
                                                        ((double) gsize));
            long long int top = totalOverlap;
            long long int bottom = expOverlap;
            if (top < 1) top = 1;
    ...
    for (int i = 0; i < numFiles; i++) {
        ...
        for (int j = 0; j < numFiles; j++) {
            ...
            if (fixedDistanceFlag != 0) {
                ...
                int numExpected = (int) ((((double) totalPeaksI) * ((double) totalPeaksJ)) / (double) genomicBins +
                                         0.5);
                ...
                long long int expOverlap = (long long int) (((double) totalCoverageI) * ((double) totalCoverageJ) /
                                                            ((double) gsize));
                long long int top = totalOverlap;
                long long int bottom = expOverlap;
                if (top < 1) top = 1;
...
//void Peak::scoreNFR(int expIndex,int fragLength,char strandInfo,int nfrSize, int nucSize) {
    ...
    for (int i = 0 - poffset - nucSize - nfrSizeHalf; i < 0 - poffset - nfrSizeHalf; i++)
        nuc1Score += cov[i - covOffset];
    for (int i = 0 - poffset - nfrSizeHalf; i < 0 - poffset + nfrSizeHalf; i++) nfrScore += cov[i - covOffset];
    for (int i = 0 - poffset + nfrSizeHalf; i < 0 - poffset + nfrSizeHalf + nucSize; i++)
        nuc2Score += cov[i - covOffset];
...
void TagLibrary::autoCorrelateTags(FILE *nfp, int windowSize, double maxTags) {
    ...
    if (mCflag == 0) {
        ...
        fprintf(fp,
                "\tAvg abs change in mC%%, Same Strand (+ for Watson strand, - for Crick)\tAvg abs change in mC%%, Opposite Strand\n");
    }
...
PeakLibrary *TagLibrary::findGroSeqTranscripts(int mode, TagLibrary *input, char *uniqMapDirectory, char strand,
                                               ...
                                               double foldTranscriptBody, double endFold, double pseudoTags,
                                               double inputFold, int groseqMethod) {
    ...
    for (int i = 0; i < chrs->total; i++) {
        ...
        PeakLibrary *ctpeaks = ct->findGroSeqTranscripts(mode, ctinput, umc, curStrand, tssSize,
                                                         ...
                                                         threshold, foldTranscriptStart, foldTranscriptBody, endFold,
                                                         inputFold,
        ...
        if (strand == STRAND_SEPARATE) {
            ...
            PeakLibrary *ctpeaks = ct->findGroSeqTranscripts(mode, ctinput, umc, curStrand, tssSize,
                                                             minBodySize, maxBodySize,
                                                             threshold, foldTranscriptStart, foldTranscriptBody,
                                                             endFold, inputFold,
                                                             inputNorm, fragmentLengthEstimate, inputFragLen,
                                                             pseudoTags, groseqMethod);
            if (ctpeaks != NULL) {
                fprintf(stderr, "+ %d-", ctpeaks->numPeaks);
                putativePeaks->addPeakLibrary(ctpeaks);
                delete ctpeaks;
...
void TagLibrary::printBedGraph(FILE *fp, double normTotalTags, char strand, int resolution, int negFlag,
    ...
    if (bedname == NULL) {
        ...
        if (style == UCSC_METHYLATED_CpG) {
            sprintf(ucscname, "%s Methylated Cytosines Total Positions = %.2e%s", name, (double) totalPositions,
                    rationame);
        } else if (style == UCSC_UNMETHYLATED_CpG) {
            sprintf(ucscname, "%s Unmethylated Cytosines Total Positions = %.2e%s", name, (double) totalPositions,
                    rationame);
        } else {
...
void TagLibrary::normalizeTagCountsGC(char *gcCtrlFile, NucleotideFreq *nf,
                                      double minNormRatio, double maxNormRatio, double gcWindow,
...
void TagLibrary::normalizeTagCountsOligos(char *genomeDirectory, Hashtable *oligos,
                                          int oligoLength, int regionStart, int regionEnd, double minFold,
                                          double maxFold, float maxPerBp) {
    //checkTagSeqBias must have been performed before
    char *fname = new char[10000];
...
void TagLibrary::assignPETagsToRestrictionSites(char *site, int maxMisMatches, char *genomeDirectory,
                                                int mode, int midFlag, int selfLigationFlag, int removeRestrictionEnds,
                                                int fragLength) {
...
void TagLibrary::makeHiCHistogram(char *outputFile, PeakLibrary *peaks, int size,
    ...
    double Navg = 0.0;
    for (int i = 0; i < histPeaks->numPeaks; i++) {
        if (histPeaks->peakOrder[i]->v < model->minCoverage || histPeaks->peakOrder[i]->v > model->maxCoverage)
            continue;
...
double **TagLibrary::makeHiCMatrix(PeakLibrary *peaks1, PeakLibrary *peaks2,
    ...
    double totalInteractions = totalTags;
    fprintf(stderr, "\tTotal Hi-C interactions: %d\n", (int) (totalInteractions / 2.0));
...
double **TagLibrary::calcCorrelationMatrix(double **matrix, int numPeaks1, double **expectedMatrix,
    ...
    for (int i = 0; i < numPeaks1; i++) {
        ...
        for (int j = 0; j < numPeaks1; j++) {
            ...
            corrMatrix[i][j] = calcCorrelation(matrix[i], matrix[j], numPeaks1, expectI, expectJ, minReads,
                                               maxIndexDiff, i, j);
...
void ChrTags::assignPETagsToRestrictionSites(char *site, Hashtable *siteHash, int fragLength, int mode, int midFlag,
                                             int removeSelfLigationFlag, int removeRestrictionEnds, double *posStrand,
                                             double *negStrand, int distLength) {
...
PeakLibrary *ChrTags::findGroSeqTranscripts(int mode, ChrTags *input, UniqMapChrs *umc, char strand,
                                            ...
                                            double foldTranscriptBody, double endFold, double inputFold,
                                            double inputNorm,
...
double ChrTags::trimGroSeqTranscript(float *profile, Peak *trans, int strand, int fragLength, double endFold,
                                     int minBodySize) {
...
void ChrTags::findmCPeaks(PeakLibrary *peaks, int peakSize, char strand, int mCflag,
        ...
        if (testPeaks[i]->score < mCthresh) {
            ...
            if (lastN > 0) {
                if (lastEnd + 1 > testPeaks[i]->start || badPeak == 0) {
                    ...
                    peaks->addPeak(NULL, chr, glastStart, glastEnd, (glastStart + glastEnd) / 2, STRAND_POSITIVE,
                                   reportStat, lastN, NULL, 0, 0);
                    chrNumPeaks++;
    ...
    if (lastN != -1 && lastStart != -1) {
        ...
        double reportStat = lastmCscore / lastN;
        if (mCflag == PEAKS_FIND_METHYLC) reportStat *= -1;
        peaks->addPeak(NULL, chr, glastStart, glastEnd, (glastStart + glastEnd) / 2, STRAND_POSITIVE, reportStat, lastN,
                       NULL, 0, 0);
        chrNumPeaks++;
        //fprintf(stdout, "%s\t%d\t%d\t%s-Region%d\t%lf\t+\n",chr,lastStart,lastEnd,chr,regionID++,lastmC/lastN);
    }
...
void ChrTags::getPETagTotals(PeakLibrary *peaks1, int &peakIndex1) {
    ...
    while (peakIndex1 < numPeaks1 &&
           chrcmp(&(peakArray1[peakIndex1]->chr), &chr) < 0)
        peakIndex1++;
    ...
    while (lastIndex1 < numPeaks1 &&
           chrcmp(&(peakArray1[lastIndex1]->chr), &chr) <= 0)
        lastIndex1++;
                ...
                if (actionFlag & HIC_MASK_NORM_DISTANCE) {
                    ...
                    if (distance < INT_MAX) {
                        if (1) {
                            ...
                            expect *= (chrModel->scaleFactor[bin1] * bin1weight +
                                       chrModel->scaleFactor[bin2] * bin2weight);
                            stdFactor = (chrModel->stdScaleFactor[bin1] * bin1weight +
                                         chrModel->stdScaleFactor[bin2] * bin2weight);
                    ...
                    if (gilInteractionIndex < gil->numInteractions &&
                        ...
                        if (gil->recordInteractionBg) {
                            ...
                            int thickness = (int) ((((double) logp) * -1.0) / 5.0 *
                                                   (10000.0 / (10000.0 + (double) resolution / 3.0)));
...
//fprintf(stderr, "%s\t%d\t%s\t%d\t%lf\n", chr, start1,  chr2, curStart, p2Stats[j]);
        ...
        if (1 && gil != NULL && actionFlag & HIC_MASK_INTERACTIONSTATSFILES && i != curNumPeaks1 - 1) {
            ...
            //double r2 = 0.0;
            double cohesinRatio =
                    log((totalTranscriptionInteractions + 1.0) / (totalCohesinInteractions + 1.0)) / log(2.0);
        ...
        for (int j = 0; j < numPeaks2; j++) {
            ...
            if (actionFlag & HIC_MASK_NORM_SEQDEPTH && model != NULL && (peakArray2[j]->v < model->minCoverage
                                                                         || peakArray2[j]->v > model->maxCoverage))
                continue;
...
int HiCBgModel::load() {
    ...
    while (fgets(buffer, BUFFER, fp) != NULL) {
        ...
        if (strcmp("Resolution=", cols[0]) == 0) {
            if (numCols < 2) {
                fprintf(stderr,
                        "\t!!! Looks like you could have an older version of a background file (going to remake)...\n");
                return 0;
            }
            int fileRes = 0;
...
GenomeInteraction::GenomeInteraction(char *iname,
                                     ...
                                     double iinteractions, double iexpected, double ilogp, double izscore,
                                     int ithickness) {
    init();
    if (iname != NULL) {
...
void GenomeInteractionLibrary::print(FILE *fp, int centerFlag) {
    ...
    fprintf(fp, "Interaction Reads\tExpected Reads\tZ-score\tLogP\tFDR(Benjamini, based on %.2le total tests)",
            totalTests);
...
void
ChrPeaks::extractSequence(FILE *fpin, FILE *fpout, int fastaFlag, int maskFlag, char *seqBuffer1, char *seqBuffer2) {
...
void NucleotideFreq::print(FILE *fp) {
    ...
    fprintf(fp,
            "Offset\tA\tC\tG\tT\tA/T\tC/G\tA/G\tC/T\tAA\tAC\tAG\tAT\tCA\tCC\tCG\tCT\tGA\tGC\tGG\tGT\tTA\tTC\tTG\tTT\tN\tNN\n");
    for (int i = 0; i < length; i++) {
        int p = i + offset;
        fprintf(fp, "%d", p);
        for (int j = 0; j < 4; j++) {
...
double NucleotideFreq::calculateGCNormalization(NucleotideFreq *ctrl, double minNormRatio,
    ...
    for (int i = 0; i < gcDistTotal; i++) {
		...
		*/
        gcNorm[i] = ratio;
        if (outfp != NULL) {
            fprintf(outfp, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", freq, gcDist[i], gcNorm[i], gcNorm[i] * gcDist[i], sum,
                    sumCtrl);
        }
    }
    if (outfp != NULL) {
